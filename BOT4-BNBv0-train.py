...
def mockup_train_net(lc_step_in: list, lc_step_out: list) -> bool:
    ...
    """
    logging.debug(
        f"Features: {len(lc_step_in)} | Shape: {lc_step_in[0].status.shape} | Feature Name: {[e_enum.name for e_enum in Perception.E_INPUT_STATUS_VECTOR]}")
    logging.debug(
        f"Features: {len(lc_step_in)} | Shape: {lc_step_in[0].mats.shape} | Feature Name: {[e_enum.name for e_enum in Perception.E_INPUT_SPACIAL_MATRICIES]}")
    logging.debug(
        f"Labels: {len(lc_step_out)} | Shape: {lc_step_out[0].mats.shape} | Label Name: {[e_enum.name for e_enum in Action.E_OUTPUT_SPACIAL_MATRICIES]}")
